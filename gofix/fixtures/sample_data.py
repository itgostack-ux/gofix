...
def create_walkin_sources():
	"""Walk-in sources are canonical masters — seed from the migrate patch."""
	print("Seeding Walk-in Sources via gofix.patches.seed_gofix_service_masters...")
	from gofix.patches.seed_gofix_service_masters import _seed_walkin_sources

	_seed_walkin_sources()


def create_issue_categories():
	"""Issue Categories are canonical masters — seed from the migrate patch.

	The taxonomy lives in gofix.patches.seed_gofix_token_masters (single
	source, shared with the GoFix Token symptom mapping). The old hardcoded
	list here used legacy names (Screen Issues, Battery Issues, …) that no
	longer match the catalogue.
	"""
	print("Seeding Issue Categories via gofix.patches.seed_gofix_token_masters...")
	from gofix.patches.seed_gofix_token_masters import _seed_issue_categories

	_seed_issue_categories()


def create_withdrawal_reasons():
	"""Withdrawal reasons are canonical masters — seed from the migrate patch."""
	print("Seeding Withdrawal Reasons via gofix.patches.seed_gofix_service_masters...")
	from gofix.patches.seed_gofix_service_masters import _seed_withdrawal_reasons
